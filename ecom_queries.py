...
querryforCouponsList="""SELECT 
	upper(trimBoth(events.couponCode)) AS coupon, COUNT(*) AS total_coupons_blocked FROM ecomm_all_events events 
	WHERE clientEvent = 1 AND eventName = 3 AND trimBoth(events.couponCode) != '' AND uuidPath is not null 
	AND (events.coupon like '%nenlahapcbofgnanklpelkaejcehkggg%'  or events.coupon like '%bmnlcjabgnpnenekpadlanbbkooimhnj%') 
	AND idx_date >= :start_date_str AND idx_date <= :end_date_str 
	AND (publisherHostname in
                (SELECT distinct lower(host)
                 FROM preagg_org_id_hosts
	...
	GROUP BY coupon ORDER BY total_coupons_blocked DESC LIMIT 5"""  
query_for_version_stats="""SELECT 
	count(version) cv, version, publisherHostname, idx_date from ecomm_all_events where idx_date > :week_ago_str and
	(publisherHostname in
                (SELECT distinct lower(host)
                 FROM preagg_org_id_hosts
                 WHERE orgId = :organizationId)
             OR publisherHostname in
            ...
            group by idx_date, version, publisherHostname
            having cv > 10
